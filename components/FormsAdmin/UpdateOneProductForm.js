...
const UpdateOneProductForm = ({ data, setProduct }) => {
  ...
  /**
   * При изменении выбранного товара:
   * - если в объекте нет images/description, добираем полный товар по /api/admin/product/[id]
   * - заполняем форму
  ...
  useEffect(() => {
    const hydrateProduct = async () => {
      ...
      try {
        if (!data.images || data.images === "[]" || data.description === undefined) {
          const res = await fetch(`/api/admin/product/${data.id}`);
          if (res.ok) {
            const json = await res.json();
            if (json?.oneProduct) {
              full = json.oneProduct;
            }
          }
        }
      } catch (e) {
      ...
      setSelectedCategory(full.categoryId);
      setSelectedGroup(full.groupId);

      if (full.images) {
        try {
          let parsedImages =
            typeof full.images === "string"
              ? JSON.parse(full.images)
              : full.images || [];
    ...
    <div className="pt-3">
      ...
      >
        ...
        <Form.Item
          name="description"
          label={
            <span style={{ color: "white" }}>Описание товара</span>
          }
        >
          <TextArea
            rows={4}
            placeholder="Описание для карточки товара (видно на сайте)"
            style={{ backgroundColor: "#191919", color: "white" }}
          />
        </Form.Item>
