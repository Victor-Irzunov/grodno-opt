...
const SortableImage = ({ id, image, onRemove, isMain }) => {
  ...
  const handleDelete = (e) => { e.stopPropagation(); onRemove(id); };
  ...
  return (
    <div className="sd:mr-2 xz:mr-1 relative">
      <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="sortable-image-item mt-4 relative">
        {imageUrl ? <img src={imageUrl} alt="upload" className="sd:w-24 xz:w-16" /> : <span>Ошибка загрузки изображения</span>}
        {isMain && (
      ...
      <button type="button" onClick={handleDelete} className="text-xs text-red-500">Удалить</button>
    </div>
  );
};
...
const AddOneProductForm = () => {
  ...
  const onFinish = async (values) => {
    ...
    formData.append("description", values.description || "");  // ← добавлено
  ...
  return (
    <div className='pt-3'>
      <Form form={form} name="createProduct" onFinish={onFinish} labelCol={{ span: 24 }} wrapperCol={{ span: 24 }}>
        <Form.Item
          name="categoryId"
          ...
          <Radio.Group>
            {categories.map(cat => (
              <Radio.Button
                key={cat.id}
                value={cat.id}
                style={{ backgroundColor: selectedCategory === cat.id ? '#0171E3' : '#191919', color: 'white', borderRadius: 0 }}
        ...
        >
          <Input type="number" size="large" style={{ backgroundColor: '#191919', color: 'white' }} />
        </Form.Item>
        ...
        <Form.Item name="status" label={<span style={{ color: 'white' }}>Статус</span>}>
          <Radio.Group>
            <Radio.Button value="В наличии" style={{ backgroundColor: '#191919', color: 'white' }}>В наличии</Radio.Button>
            <Radio.Button value="Под заказ" style={{ backgroundColor: '#191919', color: 'white' }}>Под заказ</Radio.Button>
          </Radio.Group>
        </Form.Item>
        ...
        <Form.Item label={<span style={{ color: 'white' }}>Изображения</span>} name="images">
          <Upload
            accept="image/*"
            multiple
            beforeUpload={() => false}
          ...
          >
            <Button color="primary" variant="outlined" style={{ backgroundColor: '#191919' }} icon={<UploadOutlined />}>
              ...
              <div className="sortable-images flex flex-wrap">
                {imageList.map((file, index) => (
                  <SortableImage
                    ...
                    id={file.uid}
                    image={file}
                    onRemove={(id) => setImageList(prev => prev.filter(f => f.uid !== id))}
                    isMain={index === 0}
                  />
                ))}
              </div>
