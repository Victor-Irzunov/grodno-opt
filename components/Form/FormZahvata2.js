...
const FormZahvata2 = ({ title, btn }) => {
	const [tel, setTel] = useState('');
	const [alertActive, setAlertActive] = useState(false);
	const [alertText, setAlertText] = useState('');
	const [message, setMessage] = useState(''); // поле для сообщения
	...
	const handleSubmit = (e) => {
		...
		messageForm += `<b>--------------- </b>\n`
		messageForm += `<b>Сообщение: ${message || 'пусто'} </b>\n`
		messageForm += `<b>--------------- </b>\n`
	...
	const handleChange = (e) => {
		setMessage(e.target.value); // обработчик для textarea
	};
		...
		<div className={``}>
			<form className="mx-auto" onSubmit={handleSubmit}>
				<div className='flex sd:flex-row xz:flex-col sd:space-x-10 xz:space-x-0'>
					<div className="relative z-0 w-full mb-5 group">
						<textarea
							...
							id="floating_message"
							onChange={handleChange} // изменяемый текст
				...
				<div className='my-8'>
					...
					Нажимая «Отправить» вы соглашаетесь <Link href='/politika/' className='text-primary'>с обработкой персональных данных</Link>
					</p>
