// /components/OneProductPage/OneProductPage.jsx — ФАЙЛ ПОЛНОСТЬЮ
"use client";
            ...
            {searchQuery ? (
              <ul className='text-sm font-light'>
                {searchResults && searchResults.length ? (
                  searchResults.map(el => (
                    <li key={el.id} className='p-2 border mb-1 hover:bg-sky-50 hover:translate-x-0.5 hover:shadow-lg hover-transition'>
                      <div className='flex justify-between sd:flex-row xz:flex-col'>
                        ...
                        {/* Цена/корзина для списка — оставлена без изменений */}
                      </div>
                    </li>
                  ))
                ) : (
                  <div className='text-center text-gray-300 text-xl pt-7'>
                    ...
                    <p className='text-sm mt-4'>Если вы не нашли то, что искали, позвоните нам — мы обязательно вам поможем!</p>
              ...
              <div className='grid sd:grid-cols-3 xz:grid-cols-1 gap-4'>
                ...
                <div className='sd:col-span-2 xz:col-span-1 bg-white/85 rounded-3xl sd:py-8 xz:py-5 sd:px-10 xz:px-2'>
                  ...
                  <p className='pl-2 text-gray-400 text-xs uppercase mt-2'>id: {data.id}</p>
                  ...
                  {data?.description ? (
                    <div className="mt-8 border rounded-xl bg-white/90">
                      <div className="px-4 py-3 border-b">
                        <h2 className="text-base font-semibold text-gray-800">Описание товара</h2>
                      </div>
                ...
                <article className='sd:block xz:hidden bg-slate-50 sd:py-8 xz:py-5 sd:px-6 xz:px-2'>
                  ...
                  {/* /components/OneProductPage/OneProductPage.jsx — ФРАГМЕНТ ДОБАВЛЕНИЯ БЛОКА
                      …после карточки цены/наличия, перед “Цена действительна…” */}
                  {data?.description ? (
                    <div className="mt-8">
                      <h3 className="text-sm font-semibold text-gray-700 mb-2">Описание товара</h3>
